...
You know rasters well - any digital image is a raster. While vectors connect dots with lines, rasters are grid (i.e. a matrix) of numbers, called *pixels*. The numeric value of each pixel can be used to indicate a colour (like on digital photos), or can actually represent a physical quantity, such as temperature or elevation. 

Raster files can contain multiple 'images' within them, which we call *channels* or *bands*. Digital photos for example, actually contain three bands: one specifying the amount of red colour per pixel, one for the green colour, and one for the blue colour. When you look at a photo you are seeing a *colour composition* mixing the amounts of the three primary colours according to the information held by each pixel. 
...
(@) Now let us have a look at the BIOCLIM dataset. First, read the metadata included with the data files (`Bioclim_metadata.txt`) to understand what each band of this dataset represents. Then turn on the BIOCLIM layer and turn the remaining layers off. 
...
(@) On your systems file explorer, find the base folder for today's lab according to your organisation, and right click on it. On Windows, choose `Compress to...` to create a zipfile containing your entire project. Name it as `lab_5_final_results.zip` or something similar.
