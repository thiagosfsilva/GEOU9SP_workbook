...
(@) For this exercise, we will use the the **2024** country boundaries data in **GeoJSON** format, at the **1:20** million scale, that is available from the link below. The GeoJSON format is a more recent GIS file format, commonly used for web-based mapping. It is derived from the JavaScript Object Notation (JSON) data file format, widely used to exchange data among websites and web servers. More info [here](https://en.wikipedia.org/wiki/GeoJSON). 
...
![](images/lab_2/lab2_fig1_eurostat_4326.png)

![](images/lab_2/lab2_fig1_eurostat_3035.png)
...
(@) Load the `CNTR_BN_20M_2024_4326` file into your project. Pay attention to the file name as there are many files with similar names. 
...
::: {.callout-important title="Click for answer" collapse="true" appearance="minimal"}
All file names start with `CNTR` for 'countries', followed by a two-letter code. As you've seen, `BN` seems to stand for 'boundary'(vector lines), `RG` for 'region'(vector polygons), and `LB` for 'labels' (vector points). Then `2024` specified the reference year, and `20M` indicates the 1:20 million mapping scale. The final four-digit number indicates the EPSG code for the data CRS: 4326 ('unprojected' WGS84), 3857 (WGS 84 with Pseudo-Mercator projection) or 3035 (ETRS89-extended / Lambert Azimuthal Equal Area for Europe).
:::
...
::: {.callout-important title="Stop and Think"}
a) Why doesn’t the scale shown on the bar match the “advertised” scale for the dataset (1:20 million)?
...
b) It warned you that there is more than one option for the 'on the fly' reprojection of your layer from EPSG 4326 to EPSG 3035. It showed you the options with the most accurate (1m) selected by default. But this option is only valid for Europe. 
:::
...
(@) Now add to the project the file called CNTR_BN_20M_2024_3035.geojson. Notice the different last four numbers on the file name.
...
::: {.callout-important title="Stop and Think"}
Why are the clipping results different even though the initial 3035 and 4326 layers looked perfectly aligned?
:::
...
(@) Now repeat the use of the `Clip` tool using “Reprojected” as the `Input Layer` and "clip_bounds" as the `Overlay Layer`. Rename the resulting layer to "Clipped_Reprojected". Which of the two originally clipped layers (“Clipped_3035” or “Clipped_4326”?) better matches the "Clipped_Reprojected" layer? 
