...
- Chesnut_2: 296, 2022-03-18 to 2022-03-19
...
[Convex hulls](https://en.wikipedia.org/wiki/Convex_hull) are a type of Minimum Enclosing Polygon. The best way to think of it is as if the points were pegs on a board, and you wrapped a stretch band around them. The convex hull is the shape with the *smallest perimeter* enclosing all points. 
...
(@) Now set the home range polygon symbologies to an outline that is the same colour as the original points with no fill (or a transparent fill), to evaluate the overlap in home ranges. Also add the ESRI Satellite Layer from the `QuickMapServices` plugin for reference. Mine looks like this:
