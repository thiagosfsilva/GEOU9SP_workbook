...
(@) Now use the `Origin` attribute to style the colour or shape of your Earthquake points differently for land and ocean, using the `Symbology` options. Pick the `Categorized` symbology option at the top, and then the set the `Value` to the `Origin`attribute, and then `Classify`.
...
For that, obtain the following layers from the [NatureScot Open Data Hub](https://opendata.nature.scot/):

- Wildcat Priority Areas (WPA)
- Sites of Special Scientific Interest (SSSI)

Then obtain the Open Roads layer for all of the UK from the [Ordnance Survey OS Open Data](https://osdatahub.os.uk/downloads/open) hub (it is a large file and it may take a while to download).
...
2) The UK boundaries file has a different CRS from the rest, and it also contains all countries. Reproject this layer to the same CRS as the other layers, and then create a new layer containing the Scotland boundary only. Renove the UK layer from the porject after that.
