...
Now that we have our main roads, let us add some more context to the map. But first save yoiur project!
...
(@) Go to `Project > New Print Layout`, and create a new layout called `bamburgh_map`. Then in the new Layout Editor windiow use the `Add Map` (![](https://docs.qgis.org/3.34/en/_images/mActionAddMap.png)) tool to add your map to the page. Leave some room at the right side for the legend and annotations:
