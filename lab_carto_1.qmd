...
(@) Go to the [OS Open Data Hub](https://osdatahub.os.uk/downloads/open) and find the `GB Overview Maps` collection. Click on it and download the only available file. 
...
::: {.callout-important title="Click for answer" collapse="true" appearance="minimal"}
On week 2 you learned about `Clip` and `Select by Location`. It seems that internally, when you draw a selection, Digimap is making a `Select by Location` operation, selecting all polygons that overlap with the NU square. For large polygons that cross OS grid tiles, you end up bringing the whole polygon. So now let us do a proper `Clip`. 
:::
...
(@) Now enable the Foreshore layer, and open its `Properties > Symbology`. Within the `Simple Fill` sub-symbol, change both the `Fill color` and `Stroke color` to the colour you copied from the step above, and change the `Fill style` from `Solid` to `Dense 6`. Click `OK`. You should obtain an effect similar to the image below.
