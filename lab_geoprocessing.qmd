...
You already learned and used one geoprocessing tool: the `Clip` tool. It finds the areas that are common between two layers, and 'cookie cuts' one of them based on the shape of the other. 
...
We thus need to further subset our data to match the actual extent of the Stirling council. 
...
::: {.callout-important title="Click for answer" collapse="true" appearance="minimal"}
You could use `Clip` or `Intersection` if you wanted to change ('cut') the geometries of the target layer, or use `Select by Location` if you wanted to find the overlapping polygons without changing geometries. 
:::
...
That is the end of this lab. From now on the exercises will be a bit more involved, like the one above. But at the same time you will notice that much of the work is always the same - loading data, organising it, doing queries - we will just be adding new tools that open up new (but quite specific) analysis possibilities and/or make some of the things you did before faster/simpler. Overall, the general workflow of GIS is always the same. 